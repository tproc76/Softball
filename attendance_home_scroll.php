		...
		<div class="row justify-content-md-center">
			...
			<?php
				...
				if ($display=="teams")
					...
					$table = '<td colspan=2><div id="table-scroll" class="table-scroll"><div class="table-wrap"><table id="edittable" border="1" cellspacing="1" cellpadding="5">';
					...
					if ($result = mysqli_query($link, $findGames) )
						...
						if ( $numgames > 0 )
							...
							if ($teamRole == 'M')
								{
								$table .= '<tr><td class="fixed-side" scope="col">Edit</td><td>Player</td>';
								}
							else
								{
								$table .= '<tr><td class="fixed-side" scope="col">Player</td>';
								}
							$cnt = 0;
							...
							while ($row = mysqli_fetch_array($result,MYSQLI_ASSOC))
								...
								$table .= $row['location'];
								$table .= '<br>';
								// Continued below - no add comment button if locked out already
							...
							if ($result = mysqli_query($link, $getTeamMembers) )
								...
								while ($row = mysqli_fetch_array($result,MYSQLI_ASSOC))
									...
									if ($teamRole == 'M')
										{
										$thisLine .= '<td class="fixed-side" scope="col"><button type="button" onclick="updateAttendance(' . $pid .')">Edit</button></td>';
										}
										
									$thisLine .= '<td class="fixed-side" scope="col">';
									$thisLine .= $row['name'];
									if ($row['role'] == 'M')
										{
										$thisLine .= " (M)";
							...
							if ($teamRole == 'M')
								...
								for ($col = 0; $col < $numgames; $col++)
									{
									$table .= "<td style='text-align:center' id='btn_$col'><button type='button' onclick='sendAttendEmail($teamid,$gameNums[$col])'>Send Team Email</button></td>";
									}
								}
...
<!--		<script type="text/javascript" src="dist-js/bootstrap.min.js" integrity="sha384-vBWWzlZJ8ea9aCX4pEW3rVHjgjt7zpkNpZk+02D9phzyeVkE+jo0ieGizqPLForn" crossorigin="anonymous"></script>
	...
	//										imgobj.addEventListener("touchend",handlerFunction,false);
			...
			// requires jquery library - for Fixed first column
